...
public:
	...
	template <typename T>
	bool GetChildValue( const std::string &sName, T &out ) const { const XNode *pChild=GetChild(sName); if(pChild== nullptr) return false; pChild->GetTextValue(out); return true; }
	bool PushChildValue( lua_State *L, const std::string &sName ) const;

	// modify DOM
	template <typename T>
	XNode *AppendChild( const std::string &sName, T value )	{ XNode *p=AppendChild(sName); p->AppendAttr(XNode::TEXT_ATTRIBUTE, value); return p; }
	XNode *AppendChild( const std::string &sName )		{ XNode *p=new XNode(sName); return AppendChild(p); }
	...
	template <typename T>
	XNodeValue *AppendAttr( const std::string &sName, T value ) { XNodeValue *pVal = AppendAttr( sName ); pVal->SetValue( value ); return pVal; }
