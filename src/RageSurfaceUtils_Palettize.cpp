...
#define PAM_DEPTH(p)                                                 \
  PAM_ASSIGN(                                                        \
      (p), (uint8_t)table[PAM_GETR(p)], (uint8_t)table[PAM_GETG(p)], \
      (uint8_t)table[PAM_GETB(p)], (uint8_t)table[PAM_GETA(p)])
...
static acolorhist_item* mediancut(
    acolorhist_item* achv, int colors, int sum, int maxval, int newcolors);

static bool compare_index_0(
    const acolorhist_item& ch1, const acolorhist_item& ch2) {
  return ch1.acolor[0] < ch2.acolor[0];
}

static bool compare_index_1(
    const acolorhist_item& ch1, const acolorhist_item& ch2) {
  return ch1.acolor[1] < ch2.acolor[1];
}

static bool compare_index_2(
    const acolorhist_item& ch1, const acolorhist_item& ch2) {
  return ch1.acolor[2] < ch2.acolor[2];
}

static bool compare_index_3(
    const acolorhist_item& ch1, const acolorhist_item& ch2) {
  return ch1.acolor[3] < ch2.acolor[3];
}

static acolorhist_item* pam_computeacolorhist(
    const RageSurface* src, int maxacolors, int* acolorsP);
static void pam_addtoacolorhash(
    acolorhash_hash& acht, const uint8_t acolorP[4], int value);
static int pam_lookupacolor(
    const acolorhash_hash& acht, const uint8_t acolorP[4]);
...
void RageSurfaceUtils::Palettize(
    RageSurface*& pImg, int iColors, bool bDither) {
  ASSERT(iColors != 0);
  ...
  RageSurfaceUtils::ConvertSurface(
      pImg, pImg->w, pImg->h, 32, Swap32BE(0xFF000000), Swap32BE(0x00FF0000),
  ...
  for (int row = 0; row < pImg->h; ++row) {
    ...
    do {
      ...
      if (bDither) {
        ...
        PAM_ASSIGN(
            pixel, (uint8_t)sc[0], (uint8_t)sc[1], (uint8_t)sc[2],
            (uint8_t)sc[3]);
      }
...
static acolorhist_item* mediancut(
  ...
  ASSERT(bv != nullptr);
  acolormap =
      (acolorhist_item*)malloc(sizeof(struct acolorhist_item) * newcolors);
...
#define pam_hashapixel(p)                                            \
  (((unsigned)PAM_GETR(p) * 33023 + (unsigned)PAM_GETG(p) * 30013 +  \
    (unsigned)PAM_GETB(p) * 27011 + (unsigned)PAM_GETA(p) * 24007) % \
   (unsigned)HASH_SIZE)

static bool pam_computeacolorhash(
    const RageSurface* src, int maxacolors, int* acolorsP,
...
static acolorhist_item* pam_acolorhashtoacolorhist(
    const acolorhash_hash& acht, int maxacolors) {
  // Collate the hash table into a simple acolorhist array.
  acolorhist_item* achv =
      (acolorhist_item*)malloc(maxacolors * sizeof(struct acolorhist_item));
  ASSERT(achv != nullptr);

  // Loop through the hash table.
  int j = 0;
  for (unsigned i = 0; i < HASH_SIZE; ++i) {
    for (acolorhist_list achl = acht.hash[i]; achl != nullptr;
         achl = achl->next) {
      // Add the new entry.
      achv[j] = achl->ch;
      ++j;
...
static void pam_addtoacolorhash(
    acolorhash_hash& acht, const uint8_t acolorP[4], int value) {
  acolorhist_list achl =
      (acolorhist_list)malloc(sizeof(struct acolorhist_list_item));
  ASSERT(achl != nullptr);
...
static void pam_freeacolorhist(acolorhist_item* achv) { free((char*)achv); }
