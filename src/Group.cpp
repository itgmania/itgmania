...
Group::Group(
  ...
  // below.
  m_fSyncOffset =
      (PREFSMAN->m_DefaultSyncOffset == SyncOffset_NULL) ? 0.0f : -0.009f;
  m_bHasPackIni = false;
  m_iYearReleased = 0;
  m_sBannerPath = "";
  m_sSeriesBannerPath = "";
    ...
    if (!sVersion.empty()) {
      ...
      if (!sValue.empty()) {
        if (sValue == "NULL") {
          ...
          LOG->Warn(
              "Group::Group: Invalid SyncOffset value: %s in Pack.ini file "
              "\"%s\". Valid values are NULL and ITG. Using default value.",
              sValue.c_str(), sPackIniPath.c_str());
        }
      }
      ...
      LOG->Warn(
          "Group::Group: Pack.ini version not set in file \"%s\". Using "
          "default values.",
          sPackIniPath.c_str());
    }
  }
