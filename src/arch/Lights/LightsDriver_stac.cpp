...
void LightsDriver_stac::SetBuffer(
  ...
  if (index_offset < STAC_HIDREPORT_SIZE &&
...
void LightsDriver_stac::HandleState(
  ...
  if (EqualsNoCase(sInputName, "dance")) {
    SetBuffer(
        STAC_LIGHTINDEX_BTN1, ls->m_bGameButtonLights[ctrlNum][DANCE_BUTTON_UP],
        ctrlNum);
    SetBuffer(
  ...
  if (stateChanged[ctrlNum]) {
    devs[ctrlNum].Write(
        (unsigned char*)&outputBuffer[ctrlNum], STAC_HIDREPORT_SIZE);
