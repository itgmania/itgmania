...
void CreateBinaryTestFile(std::string fn, int size) {
  RageFile f;
  ...
  if (!f.Open(fn)) {
    LOG->Warn(
        "CreateBinaryTestFile: error reopening %s: %s", fn.c_str(),
        f.GetError().c_str());
    return;
  }
...
bool CheckData(const std::string& buf, int offset) {
  char c = offset % 256;
...
int TestBinaryRead(
    RageFile& test, int size, bool AllowEOF, int ExpectedFilePos,
    int TestDataPos) {
  char buf[4096];
  ...
  if (!CheckData(std::string(buf, ret), TestDataPos)) {
    LOG->Warn(
        "TestBinaryRead: check failed (size %i, %i,%i)", size, ExpectedFilePos,
        TestDataPos);
    return -1;
  }
...
std::string MakeTextTestLine(int line, int size) {
  std::string ret = ssprintf("%02i", line);
  ret.append(std::string(size - ret.size(), 'x'));
  return ret;
}

bool CheckTextData(const std::string& buf, int LineSize, int size) {
...
void TestText(int LineSize, bool DOS) {
  ...
  if (CreateTestFiles) {
    ...
    for (unsigned line = 0; line < NumLines; ++line) {
      const std::string TestLine = MakeTextTestLine(line, LineSize);
      f.Write(TestLine);
      if (DOS) {
        f.Write("\r");
      }
  ...
  if (!test.Open(filename)) {
    LOG->Warn(
        "Open \"%s\" failed: %s", filename.c_str(), test.GetError().c_str());
    exit(1);
  }
  ...
  for (unsigned line = 0; line < NumLines; ++line) {
    ...
    if (test.GetLine(buf) <= 0) {
      LOG->Warn(
          "Text read failure, line %i, size %i, DOS %i", line, LineSize, DOS);
      return;
    }
    ...
    if (test.AtEOF()) {
      LOG->Warn(
          "Unexpected EOF in text read, line %i, size %i, DOS %i", line,
          LineSize, DOS);
      return;
    }
    ...
    if (!CheckTextData(buf, line, LineSize)) {
      LOG->Warn(
          "Text read check failure, line %i, size %i, DOS %i: '%s'", line,
          LineSize, DOS, buf.c_str());
      return;
    }
  ...
  if (ExpectedPos != test.Tell()) {
    LOG->Warn(
        "Text read pos failure, size %i, DOS %i: exp %i, got %i", LineSize, DOS,
        ExpectedPos, test.Tell());
    return;
  }
...

  {
    ...
    if (buf != "final") {
      LOG->Warn(
          "Final text read check failure, DOS %i: '%s'", DOS, buf.c_str());
      return;
    }
    if (test.AtEOF()) {
...
void TestText() {
  /* Test with these line sizes.  The internal buffer size is 256, so test line
   * sizes around there carefully. */
  int sizes[] = {4, 7, 100, 200, 254, 255, 256, 257, 258, 500, -1};
...
void TestSeek(bool relative) {
  const std::string TestLine = "Hello World\n";
...
void CheckDeflate(
    RageFileObjMem& data, int iSize, unsigned iInputCRC, int iBlockSize) {
  data.Seek(0);
...
int main(int argc, char* argv[]) {
  ...
  if (CreateTestFiles) {
    ...
    if (test.Tell() != 0) {
      LOG->Warn(
          "Simple Tell() after reopen failed (got %i, expected 0)",
          test.Tell());
      exit(1);
    }
