...
void ReadData(
    RageSoundReader* pReader, int iFrame, /* start */
    float* pBuf,                          /* out */
    int iFrames) {
  if (iFrame != -1) {
...
void compare_buffers(
    const int16_t* expect, const int16_t* got, int frames,
    int& NumInaccurateSamplesAtStart, int& NumInaccurateSamples) {
...
bool compare_buffers(
    const int16_t* expect, const int16_t* got, int frames, int channels) {
  /*
   ...
   *    on the other; this happens with resamplers after a seek, since they're
   * missing data before the seeked position (could be fixed)
   *
   * When seeking, resamplers should ideally seek slightly before the requested
   * position, read the extra data, feed it to the resampling engine, and
   * discard the extra data. Otherwise, the resampler is starting with no data,
   * and will give some low samples. (The amount of incorrect data depends on
   * the window size of the resampler.) Detect if this happens: determine if the
   * data starts very inaccurately and quickly converges.
   *
  ...
  compare_buffers(
      expect, got, frames, NumInaccurateSamplesAtStart, NumInaccurateSamples);
...
RageSoundReader* ApplyFilters(RageSoundReader* s, int filters) {
  ...
  if (filters & FILTER_RESAMPLE_FAST) {
    RageSoundReader_Resample_Good* r =
        new RageSoundReader_Resample_Good(s, 10000);
    s = r;
  }
...
/* Find "haystack" in "needle".  Start looking at "expect" and move outward;
 * find the closest. */
void* xmemsearch(
    const float* haystack, size_t iHaystackSamples, const int16_t* needle,
    size_t iNeedleSamples, int expect) {
  if (!iNeedleSamples) {
...
bool RunTests(RageSoundReader* snd, const TestFile& tf) {
...

  {
    ...
    int SilentFrames =
        FramesOfSilence(sdata, one_second_frames, snd->GetNumChannels());
    ...
    if (InitialDataSize < (int)sizeof(tf.initial)) {
      LOG->Warn(
          "Not enough (%i<%i) data to check after %i frames of silence",
          InitialDataSize, sizeof(tf.initial), SilentFrames);
      return false;
    }
    ...
    if (SilentFrames != tf.SilentFrames) {
      LOG->Trace(
          "Expected %i silence, got %i (%i too high)", tf.SilentFrames,
          SilentFrames, SilentFrames - tf.SilentFrames);
      bFailed = true;
    }
    ...
    if (!Identical) {
      ...
      float* p = (float*)xmemsearch(
          sdata, one_second_bytes, tf.later, sizeof(tf.later),
          LaterOffsetSamples * sizeof(int16_t));
      if (p) {
        int SamplesOff = p - sdata;
        int FramesOff = SamplesOff / 2;
        LOG->Trace(
            "Found half second data at frame %i (wanted %i), ahead by %i "
            "samples",
            FramesOff, LaterOffsetFrames, FramesOff - LaterOffsetFrames);
      }
      //			else
      //				dump( "foo", sdata,
      // one_second/sizeof(int16_t) );
    }
  }
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (iRet != 1) {
      LOG->Warn(
          "Fail: SetPosition(0) (%s) returned %i, expected 1", szMode, iRet);
      return false;
    }
    ...
    if (ret2 != 0) {
      LOG->Warn(
          "Fail: SetPosition(1000000) (%s) returned %i instead of 0", szMode,
          ret2);
      return false;
    }
  ...
  snd->SetPosition(iFrame);
  if (!test_read(
          snd, sdata + one_second_samples * 1 / 1000,
          one_second_frames * 1 / 1000)) {
  ...
  snd->SetPosition(iFrame);
  if (!test_read(
          snd, sdata + one_second_samples * 500 / 1000,
          one_second_frames * 500 / 1000)) {
...
int main(int argc, char* argv[]) {
  ...
  TestFile files[] = {
      /* These are all the same data, but they're encoded with different
      ...
      */
      ...
      /* The following all contain the same data; they simply have different
         tags and headers. */
      {"test MP3 44100 stereo CBR.mp3",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test MP3 44100 stereo CBR (ID3V1).mp3",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test MP3 44100 stereo CBR (ID3V2).mp3",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test MP3 44100 stereo CBR (INFO, LAME).mp3",
       1744,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xfc71, 0x23ee, 0xfb74, 0x2141}},

      {"test BASS 44100 stereo CBR.wav",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test BASS 44100 stereo CBR (ID3V1).wav",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test BASS 44100 stereo CBR (ID3V2).wav",
       592,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xef22, 0x0cd6, 0xee84, 0x0bb6}},
      {"test BASS 44100 stereo CBR (INFO, LAME).wav",
       1744,
       {0x0000, 0x0001, 0x0000, 0x0001},
       {0xfc71, 0x23ee, 0xfb74, 0x2141}},
      ...
      /* The following all contain the same data; they simply have different
         tags and headers. */
      {"test MP3 44100 stereo VBR (INFO, LAME).mp3",
       1774,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xfe43, 0x262a, 0xfd0c, 0x22bc}},
      {"test MP3 44100 stereo VBR (XING, LAME).mp3",
       622,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xef6c, 0x0cb8, 0xef10, 0x0bd2}},
      {"test MP3 44100 stereo VBR (XING, LAME, ID3V1, ID3V2).mp3",
       622,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xef6c, 0x0cb8, 0xef10, 0x0bd2}},

      {"test BASS 44100 stereo VBR (INFO, LAME).wav",
       1774,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xfe43, 0x262a, 0xfd0c, 0x22bc}},
      {"test BASS 44100 stereo VBR (XING, LAME).wav",
       622,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xef6c, 0x0cb8, 0xef10, 0x0bd2}},
      {"test BASS 44100 stereo VBR (XING, LAME, ID3V1, ID3V2).wav",
       622,
       {0xffff, 0x0000, 0xffff, 0x0000},
       {0xef6c, 0x0cb8, 0xef10, 0x0bd2}},
