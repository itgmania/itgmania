...
// NOTE: once we stop targeting anything older than Windows 8, we can drop this
// file and just call FormatMessage.
...
// DXERRMSG(DI_SETTINGSNOTSAVED_ACCESSDENIED,
// "DI_SETTINGSNOTSAVED_ACCESSDENIED") DXERRMSG(DI_SETTINGSNOTSAVED_DISKFULL,
...
DXERRMSG(
    ...
    "Unable to IDirectInputJoyConfig_Acquire because the user does not have "
    "sufficient privileges to change the joystick configuration. & An invalid "
...
DXERRMSG(
    DIERR_MOREDATA,
    "Not all the requested information fit into the buffer. & This object can "
    "only be created as an aggregated object.")
DXERRMSG(
    DIERR_NOTDOWNLOADED,
...
DXERRMSG(
    ...
    "attached to it. & At least one of the pins involved in the operation is "
    "already connected.")
DXERRMSG(
    DIERR_NOTEXCLUSIVEACQUIRED,
    "The operation cannot be performed unless the device is acquired in "
    "DISCL_EXCLUSIVE mode. & This operation cannot be performed because the "
    "filter is active.")
DXERRMSG(
    ...
    "no type-specific information has been created. & One of the specified "
...
DXERRMSG(
    ...
    "playing.  Not all hardware devices support altering the parameters of an "
    "effect while it is playing. & Two pins of the same direction cannot be "
    "connected together.")
DXERRMSG(
    DIERR_UNPLUGGED,
    "The operation could not be completed because the device is not plugged "
    "in. & The operation cannot be performed because the pins are not "
    "connected.")
DXERRMSG(
    DIERR_REPORTFULL,
    "SendDeviceData failed because more information was requested to be sent "
    "than can be sent to the device.  Some devices have restrictions on how "
    "much data can be sent to them.  (For example, there might be a limit on "
    "the number of buttons that can be pressed at once.) & No sample buffer "
    "allocator is available.")
DXERRMSG(
    DIERR_MAPFILEFAIL,
    "A mapper file function failed because reading or writing the user or IHV "
    "settings file failed. & A run-time error occurred.")
#endif
...
DXERRMSG(
    ...
    "precisely identified, no meaning should be attributed to these values "
    "other than that the driver originated the error.")
...
DXERRMSG(
    ...
    "Registry entry or DLL for class installer invalid or class installer not "
    "found.")
DXERRMSG(
    DIERR_CANCELLED,
...
DXERRMSG(
    DIERR_BADINF,
    "The INF file for the selected device could not be found or is invalid or "
    "is damaged. & The specified purpose ID can't be used for the call.")
#endif
...
DXERRMSG(
    D3DERR_PRESENT_STATISTICS_DISJOINT, "Presentation statistics are disjoint")
#endif
...
DXERRMSG(
    DSERR_ALLOCATED,
    "The call failed because resources (such as a priority level) were already "
    "being used by another caller")
DXERRMSG(
...
DXERRMSG(
    ...
    "succeed")
// DXERRMSG(DSERR_OUTOFMEMORY, "Not enough free memory is available to complete
// the operation")
...
DXERRMSG(
    ...
    "The GUID specified in an audiopath file does not match a valid MIXIN "
    "buffer")
DXERRMSG(
    DSERR_OBJECTNOTFOUND,
    "The object requested was not found (numerically equal to "
    "DMUS_E_NOT_FOUND)")
...
DXERRMSG(
    ...
    "The target window or output has been occluded. The application should "
    "suspend rendering operations if possible.")
...
DXERRMSG(
    ...
    "version of the DXGI.DLL will provide run-time debug output with further "
    "information.")
DXERRMSG(
    DXGI_ERROR_NOT_FOUND,
    "The item requested was not found. For GetPrivateData calls, this means "
    "that the specified GUID had not been previously associated with the "
    "object.")
DXERRMSG(
...
DXERRMSG(
    ...
    "The application attempted to perform an operation on an DXGI output that "
    "is only legal after the output has been claimed for exclusive owenership.")
DXERRMSG(
